...
if size(filepath,1)>0
    ...
    if isempty(derived)==0   %derivatives were calculated
        ...
        if displaywhat==1 %vectors only
            ...
            if get(handles.filtersdisp,'value')==1
                currentimage = preproc(currentimage);
            end
...
function stab_Callback(hObject, eventdata, handles)
switchui('multip20')
...
if ok==1
  ...
  %{
    ...
    frame_spacing_corr = get(handles.frame_spacing_corr,'String');
    
    frame_spacing_corr = str2num(frame_spacing_corr); % Convert to integer
        
    if isempty(frame_spacing_corr) % If not an integer
        frame_spacing_corr = 1
    end
    if is_fftmulti==1
        passes=1;
...
if size(resultslist,2)>=frame
    ...
    if numel(u>0)
        ...
        if get(handles.SNR_filter, 'value')==1
            if isempty(ismean) || ismean(frame,1) ~= 1
...
save('-v7.3', fullfile(PathName,FileName), '-struct', 'app')
...
streamlamount=get(handles.streamlamount, 'string');
streamlcolor=get(handles.streamlcolor, 'value');
save('-v7.3', fullfile(PathName,FileName), '-append');
...
if isempty(resultslist)==0
    if size(filepath,1)>0
        ...
        if selectionok==1
            ...
            for count=mincount:size(resultslist,2)
                if size(resultslist,2)>=count && numel(resultslist{1,count})>0
                    [x,y,u,v,typevector] = getxyuv(resultslist,count);
...
function loadingsvidbutton_Callback(hObject, eventdata, handles)
if ispc==1
    pathname=[retr('pathname') '\'];
else
    pathname=[retr('pathname') '/'];
...
displogo(0)
if ispc==1
    path=uipickvids ('FilterSpec', pathname, 'REFilter', '\.bmp$|\.jpg$|\.tif$', 'numfiles', [1 1], 'output', 'struct', 'prompt', 'Select images. Images from one set should have identical dimensions to avoid problems.');
else
...
if isequal(path,0) ==0
    ...
    imglimit = retr('imglimit'); % retrieve the image limit 
    if imglimit <= 1
        imglimit = Inf;
    end
    grayscaling = retr('grayscalingval');
    ...
    if sequencer==1
        for i=1:size(imgpath,1)
            if exist('filepath','var')==0 %first loop
                filepath{1,1}=imgpath{i,1};
            else
                filepath{i,1}=imgpath{i,1};
            end
        end
    else %sequencer=0
        for i=1:size(path,1)
            if exist('filepath','var')==0 %first loop
                ...
                filepath{size(filepath,1)+1,1}=imgpath{i,1};
                filepath{size(filepath,1)+1,1}=imgpath{i,1};
            end
        end
    end
    ...
    if size(filepath,1) >= 1
        ...
        for i=1:size(filepath,1)
            if mod(i,2) == 1
                filename{i,1}=['A: ' filepath{i,1}];
            else
                filename{i,1}=['B: ' filepath{i,1}];
            end
        end
        %extract path:
        put ('filename',filename); %only for displaying
...
% --------------------------------------------------------------------
function Correlation_mean_Callback(hObject, eventdata, handles)
% hObject    handle to Correlation_mean (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
switchui('multip23')


% --- Executes on button press in SNR_weights.
function SNR_weights_Callback(hObject, eventdata, handles)
% hObject    handle to SNR_weights (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of SNR_weigh

function [x,y,u,v,typevector] =  getxyuv(resultslist,count)
    x=resultslist{1,count};
    y=resultslist{2,count};
    if size(resultslist,1)>6 %filtered exists
        if size(resultslist,1)>10 && numel(resultslist{10,count}) > 0 %smoothed exists
...
function [out_color] = preproc(img)
    handles = gethand;
    clahe=get(handles.clahe_enable,'value');
    highp=get(handles.enable_highpass,'value');
    %clip=get(handles.enable_clip,'value');
    ...
    [out,out_color] = PIVlab_preproc(img,roirect,clahe, clahesize,highp,highpsize,intenscap,wienerwurst,wienerwurstsize,lmt,lmtsize);
...
% --- Executes on button press in filtersdisp.
function filtersdisp_Callback(hObject, eventdata, handles)
% hObject    handle to filtersdisp (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of filtersdisp
