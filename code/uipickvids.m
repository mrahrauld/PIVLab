...
uicontrol('Position',[10 460+30 160 16],...
 ...
 refilter_ed = uicontrol('Position',[10 440+30 160 20],...
    ...
    'Visible', 'on',...
    'tooltipstring','Example:  \.m$|\.mat$  will list files ending in ''.m'' and ''.mat''.');
...
uicontrol('Position',[10 500+30 160 16],...
	...
	'HorizontalAlignment','left',...
    'Visible', 'on') 
 img_limit = uicontrol('Position',[10 480+30 160 20],...
	'Style','edit',...
	'BackgroundColor','w',...
	'String','',...
	'HorizontalAlignment','left',...
	'Callback',@setimglimit,...
    'Visible', 'on',...
    'tooltipstring','Example: Write 10 to take the 10 first images');
...
try
	...
	function done(varargin)
        ...
        imglimit = get(img_limit,'String');
        if isempty(imglimit) % If default value
            imglimit = '-1';
        end
        imglimit = str2num(imglimit); % Convert to integer
        
        if isempty(imglimit) % If not an integer
				msg = {'Image limit must be an integer.','Please change its value'};
				uiwait(errordlg(msg,'','modal'))
				return
        else
            hgui=getappdata(0,'hgui');
            setappdata(hgui, 'imglimit', imglimit);
        end
		delete(fig)
	end
